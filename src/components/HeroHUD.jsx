...
export default function HeroHUD() {
  return (
    <section className="relative min-h-[90vh] w-full overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(255,255,255,0.08),transparent_50%),linear-gradient(120deg,rgba(245,158,11,0.08),transparent_40%),linear-gradient(0deg,rgba(255,255,255,0.04),transparent_60%)]" />
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center px-4 pt-20 sm:pt-24">
        ...
        >
          <button className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-3 font-semibold text-slate-900 shadow-[0_10px_40px_-10px_rgba(245,158,11,0.8)] transition hover:translate-y-[-1px] hover:bg-amber-400 active:translate-y-0 sm:w-auto">
            <Hammer className="h-5 w-5" />
            Post a job
          </button>
          <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/10 px-5 py-3 font-semibold text-white/90 backdrop-blur-md transition hover:bg-white/20 sm:w-auto">
        ...
        <div className="relative mt-12 grid w-full grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { icon: <CreditCard className="h-4 w-4" />, label: 'Escrow milestones' },
            { icon: <Bot className="h-4 w-4" />, label: 'AI job matching' },
            { icon: <Hammer className="h-4 w-4" />, label: 'Daily work logs' },
            { icon: <Shield className="h-4 w-4" />, label: 'KYC + licenses' },
          ].map((item, i) => (
            <motion.div
              key={i}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-[-10%] h-[40%] bg-gradient-to-t from-amber-500/20 via-transparent to-transparent blur-[60px]" />
    </section>
  );
}
